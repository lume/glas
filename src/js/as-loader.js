...
function preInstantiate(imports) {
	...
	env.abort =
		...
		function abort(mesg, file, line, colm) {
			...
			throw Error(
				'abort: ' + getString(memory, mesg) + ' at ' + getString(memory, file) + ':' + line + ':' + colm
			)
		}
	env.trace =
		env.trace ||
...
function postInstantiate(baseModule, instance) {
	...
	function __getArrayView(arr) {
		...
		return getView(align, info & VAL_SIGNED, info & VAL_FLOAT).slice((buf >>>= align), buf + length)
	}
...
export {instantiateBuffer, instantiate, instantiateStreaming, demangle}
