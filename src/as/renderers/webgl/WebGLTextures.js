...
function WebGLTextures(_gl, extensions, state, properties, capabilities, utils, info) {
	...
	function resizeImage(image, needsPowerOfTwo, needsNewCanvas, maxSize) {
		...
		if (image.width > maxSize || image.height > maxSize) {
			scale = maxSize / Mathf.max(image.width, image.height)
		}
	...
	function setTextureParameters(textureType, texture, supportsMips) {
		...
		if (extension) {
			...
			if (
				...
				(capabilities.isWebGL2 || extensions.get('OES_texture_half_float_linear')) === null
			)
				return
			...
			if (texture.anisotropy > 1 || properties.get(texture).__currentAnisotropy) {
				_gl.texParameterf(
					...
					Mathf.min(texture.anisotropy, capabilities.getMaxAnisotropy())
				)
	...
	function getRenderTargetSamples(renderTarget) {
		return capabilities.isWebGL2 && renderTarget.isWebGLMultisampleRenderTarget
			? Mathf.min(capabilities.maxSamples, renderTarget.samples)
			: 0
	}
